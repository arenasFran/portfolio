...
export const Projects = () => {
  ...
  const renderAction = (label: string, href?: string, isSecondary = false) => {
    ...
    return (
      <a
        className={`${baseClass} hover:text-accent flex items-center gap-1 group`}
        href={normalizedHref}
        target="_blank"
        rel="noreferrer"
      >
  ...
  return (
    <section
      className="w-[min(760px,calc(100%-2rem))] mx-auto animate-[intro-fade_360ms_ease-out]"
      aria-label="Projects"
    >
      <h1 className="mx-auto text-center font-[family-name:var(--font-display)] text-[clamp(2.3rem,4.6vw,3.1rem)] leading-none text-accent">
        My projects
      </h1>
      <div className="mt-8 grid gap-5">
        {projects.map((p) => (
          <article
            key={p.id}
            className="border border-line rounded-2xl p-6 bg-white/[0.3] shadow-[0_8px_24px_rgba(66,50,36,0.06)] transition-all duration-300 hover:-translate-y-1 hover:border-[#c9b7a3] hover:shadow-[0_12px_32px_rgba(66,50,36,0.1)]"
          >
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="m-0 text-[clamp(1.35rem,2.8vw,1.65rem)] leading-[1.2]">
                  {p.title}
                </h2>
                {p.freelance && (
            ...
            <p className="mt-3 text-muted text-[1.05rem] leading-relaxed">
              {p.description}
            </p>
