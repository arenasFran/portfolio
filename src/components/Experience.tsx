...
const experiences: ExperienceItem[] = [
  {
    ...
    description:
      "Providing technical support and maintenance for internal systems, assisting with troubleshooting, hardware setup, and ensuring smooth day-to-day IT operations.",
  ...
  },
  {
    ...
    date: "Mar 2026",
    description:
      "Developed a product catalog web application with a focus on performance and usability, simulating an e-commerce experience without checkout by integrating direct WhatsApp contact for inquiries.",
...
export const Experience = () => {
  ...
  return (
    <section
      className="w-[min(760px,calc(100%-2rem))] mx-auto px-4 py-8 sm:px-0"
    ...
    >
      ...
      <div ref={timelineRef} className="relative mt-14">
        <div
          className="absolute left-[0.45rem] top-2 bottom-2 w-px bg-line"
          aria-hidden="true"
        />
        <motion.div
          aria-hidden="true"
              ...
              >
                <div className="flex flex-col items-start gap-1.5">
                  ...
                  <span className="inline-flex items-center gap-2 text-[12px] uppercase tracking-[0.11em] text-muted/75 transition-colors duration-200 group-hover:text-muted">
                    <span
                      className={`size-1.5 rounded-full ${item.current ? "bg-[#4CAF7D]" : "bg-[#b7b7b7]"}`}
                    />
                    {item.date}
                  </span>
                </div>
